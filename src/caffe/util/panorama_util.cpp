...
int WarpCoord2Pano(Mat& PanoImg, Mat& WarpImg, double yaw, double pitch, std::vector<double>& coords, std::vector<double>& result)
{
  ...
  double panoWidthOffset = (CV_PI-yaw)/(2*CV_PI) * static_cast<double>(panoWidth);
  double panoHeightOffset = (CV_PI/2-pitch)/CV_PI * static_cast<double>(panoHeight);
...
int WarpCoord2Pano2(Mat& PanoImg, Mat& WarpImg, int width, int height, double hFOV, double yaw, double pitch, double roll, std::vector<double>& coords, std::vector<double>& result)
{
  ...
  Mat rotatMat = rotatMatYaw.inv()*rotatMatPitch.inv()*rotatMatRoll.inv();
  Mat tmpCoor = Mat::zeros(3, 1, CV_64F);
  double F  = (warpWidth/2)/tan(hFOV/2.f);
  double du = panoWidth/2/CV_PI;
  double dv = panoHeight/CV_PI;
  for(std::vector<double>::iterator coord = coords.begin(); coord != coords.end(); coord++){
    tmpCoor.at<double>(0) = static_cast<double>((*coord++) - warpWidth/2);
    tmpCoor.at<double>(1) = static_cast<double>(warpHeight/2 - *coord);
    tmpCoor.at<double>(2) = F;
    tmpCoor = rotatMat*tmpCoor;
    double latitude  = asin(tmpCoor.at<double>(1));
    double longitude = atan2(tmpCoor.at<double>(0), tmpCoor.at<double>(2));
    double coord_w = longitude*du + panoWidth/2;
    double coord_h = panoHeight/2 - latitude*dv; 
    result.push_back(coord_w);
    result.push_back(coord_h);
  }

  //for(std::vector<double>::iterator coord = coords.begin(); coord != coords.end(); coord++){
  //  tmpCoor.at<double>(0) = *coord++;
  //  tmpCoor.at<double>(1) = *coord;
  //  tmpCoor.at<double>(2) = 0;
  //  tmpCoor = rotatMat*tmpCoor;
  //  result.push_back(tmpCoor.at<double>(0));
  //  result.push_back(tmpCoor.at<double>(1));
  //}
...
void MakeWarpImgList(cv::Mat& srcPanoImg, std::vector<cv::Mat>& warpImgList, std::vector<double> params)
{
  ...
  for(std::vector<double>::iterator iter = params.begin(); iter != params.end(); iter++){
    PanoImg2Warp(srcPanoImg, dst, static_cast<int>(*(iter++)), static_cast<int>(*(iter++)), *(iter++), *(iter++), *(iter++), *(iter++));
